...
test("parseRunLog keeps unterminated segments with null exit code", () => {
  ...
  // Newer wrapper appends "(workdir: ...)" to the segment header.
  const workdirSegments = parseRunLog("=== 2026-08-18T19:10:01+08:00 run (workdir: /home/u/.local/state/pi-async) ===\nbody\n=== exit=0 ===\n");
  assert.equal(workdirSegments.length, 1);
  assert.equal(workdirSegments[0].exitCode, 0);
